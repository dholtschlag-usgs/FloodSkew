...
# http://nwis.waterdata.usgs.gov/nwis/peak?site_no=04097500&agency_cd=USGS&format=rdb
myUrla <- "http://nwis.waterdata.usgs.gov/nwis/peak?site_no="
myUrlb <- "&&agency_cd=USGS&format=rdb"
...
for (i in 1:nGagesUse){
  ...
  writeLines(peakTable, con = paste("./data/Raw/pk",gagesUse[i],".rdb",sep=""))
}
#
